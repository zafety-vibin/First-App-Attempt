...
Auto-generated from feature plans. Last updated: 2025-10-12
...
- **015-create-the-dashboard** (2025-01-13): Interactive Dashboard Canvas System. Full-stack feature creating drag-and-drop widget canvas for campaign dashboard and category landing pages. Backend: 2 new tables (dashboard_configs, category_landing_configs with layout JSONB), 8 API routes (GET/POST/PUT/DELETE for both configs). Frontend: DashboardPage with react-grid-layout (12-column grid, 10px rows, responsive breakpoints lg/md/sm), 13 CategoryLandingPage components with CategoryLandingCanvas and CategoryLandingTextEditor (TipTap rich text), WidgetRegistry with 7 real widgets (NPC Summary, Location Explorer, Faction Power, Quest Tracker, Session Timeline, Player Characters, Recent Activity), BaseWidget wrapper with drag handle and remove button, WidgetPicker modal, size-adaptive rendering (compact for 1x1/2x2, detailed for 3x3+), auto-save with 500ms debounce, ViewModeToggle with eye icon (open=DM, closed=Player), information filtering integration, reset layout functionality. Widget data service with 7 API integrations fetching from Feature 014 category tables. Grid layout persists per user per campaign. Canvas features: drag-drop reordering, resize with grip handle (1x1 minimum to 12x50 maximum), delete widgets, reset to default layout. Category landing pages: filtered widget picker (only relevant widgets per category), rich text editor for category descriptions, Database button linking to table views (/campaigns/:id/:category/database routes). Routing: 26 routes total (13 landing pages at /campaigns/:id/:category, 13 database views at /campaigns/:id/:category/database). Technologies: react-grid-layout 1.3.4, react-resizable 3.0.4, TipTap 2.x, Better-SQLite3, Express middleware. Performance: <500ms canvas load, <100ms widget resize, 500ms debounced auto-save. Testing: Manual testing complete, automated tests pending (T034-T045). Components: DashboardPage, CategoryLandingCanvas, CategoryLandingTextEditor, BaseWidget, WidgetPicker, ViewModeToggle (eye icon), 7 widget components, services (dashboardConfigService, categoryLandingConfigService, widgetDataService), hooks (useDashboardCanvas, useCategoryLandingCanvas). Migration: 015-dashboard-canvas.sql creates both tables with proper indexes and foreign keys.
