...
- **009-create-player-question** (2025-11-23): ✅ **COMPLETE** - Player Question Portal with AI-powered Q&A. Public portal (NO auth) at /portal/:campaignId for players to ask campaign questions. GM management at /campaigns/:campaignId/portal/management (protected). 5 backend services: PortalConfigService (enable/disable, password protection, response styles), PortalPlayerService (unique character names, crypto session tokens), PortalAIService (viewMode filtering, BYOLLM integration, citation generation), PortalTokenTrackerService (per-player usage monitoring), PortalConversationService (message history). Information filtering: reuses Feature 004 viewMode system with player_view mode - ZERO dm-secret content accessible (security audit passed T060). BYOLLM integration: Feature 008 EXCLUSIVE to portal (GM's LLM credentials, OAuth 2.0 + PKCE, AES-256-GCM encryption). Citations: numbered [1][2][3] links to /cards/{cardId} from Feature 003. Response styles: 5 options (friendly-sage, scholarly-tome, tavern-gossip, factual, custom). Frontend: 13 components (7 GM portal, 6 player portal), 2 API client services, sidebar tab, routing. Backend: 5 database tables (portal_configs, portal_players, portal_conversations, portal_messages, portal_token_usage), 2 route files (management protected, public open), crypto random tokens (64-char hex), bcrypt password hashing (10 salt rounds), HTTP-only cookies for session persistence. Testing: 9 unit tests (tokens, unique names, password, citations, tracking), 4 integration tests (password flow, token persistence, filtering, card links), 3 component tests, 4 E2E tests (complete flow, password protection, information filtering, citation navigation). Performance: <100ms context build (batch fetching), player_view filtering at SQL layer. Technologies: Better-SQLite3, bcrypt, crypto, BaseCategoryService (Feature 014 standardization), informationFilter middleware (Feature 004). Status: T001-T060 complete (62/62 tasks), security audit approved, ready for localhost testing. [See spec](specs/009-create-player-question/)

- **021-create-a-geographic** (2025-10-28): ✅ **COMPLETE** - Spatial Geographic Navigator with scale-based visualization. 3 modes: Ellipse distribution (flat widescreen layout for unpinned nodes), Map Mode (parent location maps as backgrounds with pinned children as green circles), Grid View (side-by-side map comparison). Click-to-place pin workflow: select node in sidebar → click map → MapCanvas calculates exact coordinates accounting for pan/zoom/scale. Full breadcrumb path: "Plane View → Material Plane (plane) → World of Geux (world) → Solus (continent)". Edit pins mode: unpin nodes, see coordinates in overlay list. Client-side caching: loads all 51 nodes once, instant navigation between scales. Map caching: parent locations cached, second visit instant. Location existence checking: sidebar shows 📍 (can pin) vs ⚠️ (create in database first). Performance: <100ms scale transitions (cached), <1s initial load. Technologies: Konva.js (MapCanvas native pins), React hooks (client state), Better-SQLite3 (map_pin_x/y columns). Backend: GeographicNavigatorService, spatial-navigator routes, null coordinate support for unpinning. Frontend: click-to-place (not drag-drop), MapCanvas integration, breadcrumb history. Status: T001-T043 complete, T044-T046 E2E tests created, ready for spatial worldbuilding. [See spec](specs/021-create-a-geographic/)
