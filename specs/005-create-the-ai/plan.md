...
**Branch**: `005-create-the-ai` | **Date**: 2025-10-01 | **Spec**: [spec.md](./spec.md)
...
- **MCP SDK**: Model Context Protocol client library for streaming LLM operations
- **LLM Providers**: OpenAI SDK, Anthropic SDK (user-provided credentials)
- **Entity Extraction**: LLM-based NER with custom prompts
- **Graph Storage**: SQLite JSON1 extension for graph JSONB storage
...
**Storage**:
- SQLite3 (extends features 002-004 schema)
- New tables: `import_sessions`, `planning_sessions`, `knowledge_graphs`, `graph_nodes`, `graph_edges`, `import_batches`
- JSONB columns for: graph data, chat history, approval summaries
...
- LLM Testing: Mock MCP responses with fixtures
...
- MCP streaming must handle 10k+ token responses
...
- MCP protocol uses user-provided API keys (FR-010)
- Feature 008 (BYOLLM Configuration) is hard dependency for Feature 005

**BLOCKING DEPENDENCY**: Feature 005 cannot function without Feature 008 implemented first. Recommend planning order: 005 → 006 → 007 → **008 (implement)** → 005 (implement).
...
**Critical Dependency**: Feature 008 (BYOLLM Configuration) must be implemented before Feature 005 can function.
...
│   │   ├── KnowledgeGraph.ts       # NEW: Graph container (type, updated_at)
│   │   ├── GraphNode.ts            # NEW: Entity in graph (character, location, event)
│   │   ├── GraphEdge.ts            # NEW: Relationship between nodes
...
│   │   ├── MCPService.ts           # NEW: MCP protocol client, streaming, LLM calls
│   │   ├── ImportService.ts        # NEW: Entity extraction, deduplication, placement logic
│   │   ├── PlanningService.ts      # NEW: Graph updates, context assembly, consistency checks
│   │   ├── GraphService.ts         # NEW: CRUD for graphs/nodes/edges, relationship inference
│   │   ├── EntityExtractionService.ts  # NEW: LLM-based NER for characters/locations/factions
│   │   ├── TimelineService.ts      # NEW: Session Recaps timeline validation
│   │   └── FileParse Service.ts    # NEW: PDF/DOCX/MD parsing
│   ├── middleware/
...
│   │   ├── import.ts               # NEW: POST /import/upload, /import/clarify, /import/approve, /import/revert
...
│   └── db/
│       ├── schema.sql              # Extended: import_sessions, planning_sessions, knowledge_graphs, graph_nodes, graph_edges, import_batches
...
└── tests/
    ...
    ├── integration/
    │   ├── import-workflow.test.ts  # NEW: Full Import flow with mocked LLM
    ...
    └── unit/
        ...
        └── mcp-service.test.ts        # NEW: MCP streaming, error handling
...
│   │   └── SettingsPage.tsx        # (Feature 004) - will extend for AI config (Feature 008)
│   ├── services/
...
│   │   └── useMCPStreaming.ts      # NEW: Handle MCP streaming responses
│   ├── contexts/
...
│       └── mcp.ts                  # NEW: MCP client configuration
└── tests/
    ├── components/
...
**Structure Decision**: Web application (frontend + backend). Extends existing VVD-mimic architecture from features 002-004. Import/Planning workflows are frontend pull-down tabs that communicate with backend MCP services. Knowledge graphs stored in SQLite backend, exposed via REST API for frontend inspection/editing.
...
1. **MCP Protocol Integration**
   - Research MCP SDK for Node.js and browser
   - Streaming LLM responses with progress indicators
   - Bulk operation patterns (batch card creation)
   - Error handling and retry strategies
   - Decision: MCP client library vs raw HTTP/SSE

2. **Pull-Down Tab UI Pattern**
...
4. **Knowledge Graph Storage in SQLite**
   - JSONB columns for graph data vs normalized tables (nodes, edges)
   - Graph query patterns (BFS, DFS, shortest path for relationship inference)
   - Index strategies for graph traversal performance
   - Political-Web "active party-relevant" filtering algorithm
   - Decision: Single JSONB column vs node/edge tables vs external graph DB

5. **Timeline Consistency Validation**
...
10. **Graph Filtering: "Active Party-Relevant"**
    - Political-Web: Include relationships if mentioned in last N sessions (N=5?)
    - Campaign-Story: Include story threads marked "active" or referenced recently
    - Automatic pruning vs manual GM curation
    - UI for marking relationships/threads as "inactive"
    - Decision: Time-based (last N sessions) vs tag-based (active/inactive) vs hybrid
...
1. **data-model.md**:
   ...
   - KnowledgeGraph entity (type enum, JSONB data, last_updated)
   - GraphNode entity (graph FK, type, name, attributes JSONB, source card FK)
   - GraphEdge entity (from/to node FKs, relationship type, strength, source card FK)
...
2. **contracts/** (OpenAPI 3.0):
   - `import.yaml`:
     ...
     - POST /import/chat (session_id, message → clarification response)
   ...
   - `planning.yaml`:
     - POST /planning/chat (session_id, message, graph_context_flags → response)
...
4. **CLAUDE.md update**:
   - Add MCP SDK, LLM provider SDKs, pdf-parse, mammoth to dependencies
...
2. **From Data Model** (data-model.md):
   ...
   - 7 new entities (ImportSession, PlanningSession, KnowledgeGraph, GraphNode, GraphEdge, ImportBatch, AIApprovalSummary) → 7 model tasks + 1 migration task
   - Default template schemas → template SQL generation task

3. **From Services** (research.md decisions):
   - MCPService (MCP client, streaming) → 2 tasks (setup, error handling)
   - ImportService (entity extraction, deduplication, placement) → 4 tasks
   - PlanningService (graph updates, context assembly) → 3 tasks
   - GraphService (CRUD, relationship inference) → 3 tasks
   - EntityExtractionService (LLM prompts, NER) → 2 tasks
...
1. **Database layer first** (enables all other work):
   - Migration 005 (create tables) → MUST be task #1
   - Model creation tasks [P] → Tasks #2-8 (parallel)
   - Default template SQL generation → Task #9

2. **Backend services** (TDD: tests before implementation):
   - Contract tests [P] → Tasks #10-35 (parallel, one per endpoint + service method)
   - MCPService → Tasks #36-37 (setup MCP client, streaming, error handling)
   - FileParseService → Task #38 (PDF/DOCX/MD parsing)
   - EntityExtractionService → Tasks #39-40 (NER prompts, deduplication)
   - TimelineService → Task #41 (Session Recaps validation)
   - GraphService → Tasks #42-44 (CRUD, relationship inference, filtering)
   - ImportService → Tasks #45-48 (entity extraction, placement logic, approval summary generation, batch execution)
   - PlanningService → Tasks #49-51 (graph update detection, context assembly, consistency checks)

3. **Backend routes** (implement after services):
   - import.ts routes → Tasks #52-56 (upload, chat, approval-summary, approve, revert)
   - planning.ts routes → Tasks #57-59 (chat, update-graphs, session-status)
   - knowledge-graphs.ts routes → Tasks #60-64 (list graphs, get graph, PATCH node, DELETE node, POST node)

4. **Frontend core** (contexts, hooks, services):
   - ImportContext, PlanningContext → Tasks #65-66 [P]
   - useImportSession, usePlanningSession, useGraphs, useMCPStreaming hooks → Tasks #67-70 [P]
   - importService, planningService, graphService API clients → Tasks #71-73 [P]

5. **Frontend components** (UI implementation):
   - ImportTab layout → Task #74
   - ImportTab chat integration → Task #75
   - PlanningTab layout → Task #76
   - PlanningTab chat integration → Task #77
   - ApprovalSummary display → Task #78
   - ApprovalSummary interaction (modify, approve) → Task #79
   - GraphViewer (list view) → Task #80
   - RevertButton → Task #81

6. **Integration & E2E tests** (validate full workflows):
   - Import session recap E2E → Task #82
   - Import character notes E2E → Task #83
   - Planning graph update E2E → Task #84
   - Import revert E2E → Task #85
   - Graph viewer E2E → Task #86
   - Custom AI instructions E2E → Task #87

**Estimated Output**: 87 numbered, dependency-ordered tasks in tasks.md
...
- Feature 008 (BYOLLM Configuration) must be implemented BEFORE Feature 005
...
- ⚠️ **Feature 008 (BYOLLM Configuration) - BLOCKING** - User API credentials required for LLM calls
...
1. Plan Features 005-010 (all planning work upfront)
2. Implement Feature 008 (BYOLLM Configuration) FIRST
3. Implement Feature 005 (AI Import & Planning) - now unblocked
4. Implement Features 006-007 (can parallelize)
5. Implement Features 009-010 (depend on 005 Planning AI)
