...
**Feature**: 005-create-the-ai | **Date**: 2025-10-01
**Status**: Phase 0 Complete

---

## Research Area 1: MCP Protocol Integration

### Problem Statement
Need streaming LLM operations for Import entity extraction (5000+ words), Planning graph updates (50+ imported cards), and real-time chat interfaces. System must handle 10k+ token responses, support bulk operations with progress indicators, and gracefully handle API failures with retry logic.

### Research Findings

**MCP (Model Context Protocol)** is designed for structured LLM operations with streaming, context management, and bulk processing.

**Available SDKs**:
- `@modelcontextprotocol/sdk` (official TypeScript SDK for Node.js and browser)
- `openai` SDK (native streaming with SSE)
- `@anthropic-ai/sdk` (Claude streaming)

**Key Decision**: Use **OpenAI/Anthropic SDKs directly** instead of MCP SDK for prototype simplicity.
...
**Bulk Operation Pattern** (Entity Extraction):
```typescript
...
`;
  ...
  const response = await mcpService.completion(prompt, systemPrompt);
...
export async function completionWithRetry(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      return await mcpService.completion(prompt, systemPrompt);
    } catch (error) {
...
### Decision
**Use OpenAI/Anthropic SDKs directly** for prototype. No MCP SDK wrapper needed. Implement streaming via native SDK support (AsyncGenerator for backend, ReadableStream for frontend). Retry logic with exponential backoff for 429/5xx errors. Progress indicators via phase-based updates (not true streaming progress).

### Alternatives Considered
- **@modelcontextprotocol/sdk**: Too complex for prototype, adds abstraction layer we don't need yet. Future consideration if multi-provider support required.
- **LangChain**: Heavy framework, overkill for structured prompts. Rejected.
- **Server-Sent Events (SSE)**: More complex than SDK native streaming. Rejected.

### Implementation Notes
- **API Key Storage**: Use BYOLLM config from Feature 008, stored in `settings` table (encrypted at rest)
- **Model Selection**: Support GPT-4, GPT-3.5-turbo, Claude 3.5 Sonnet via dropdown in Settings
...
### Problem Statement
Import AI must extract characters, locations, factions, events from unstructured notes, then deduplicate against existing campaign cards to distinguish updates from new additions. Need fuzzy matching ("Dagult Neverember" vs "Lord Neverember"), confidence scoring, and multi-pass extraction strategy.
...
**Multi-Pass Extraction** (Single comprehensive pass preferred for prototype):
```typescript
// REJECTED: Multi-pass extraction (too slow for prototype)
// Instead: single comprehensive pass with high token limit
...
export class TimelineService {
  ...
  async extractTimeline(notes: string): Promise<SessionRecapEntry> {
    ...
    const response = await this.mcpService.completion(prompt, 'You are a timeline extraction assistant.');
    return JSON.parse(response);
  }
  ...
  async checkConsistency(
    ...
    const response = await this.mcpService.completion(
      prompt,
...
export async function importSessionRecap(
  ...
  const timelineService = new TimelineService(mcpService);
...
export async function assemblePlanningContext(
  ...
  const graphs = await Promise.all([
    graphService.getGraph('geographical', campaignId),
    graphService.getGraph('political-web', campaignId),
    graphService.getGraph('world-foundations', campaignId),
    graphService.getGraph('campaign-story', campaignId),
  ]);

  // Filter active nodes for Political-Web/Campaign-Story
  const politicalWebActive = await graphService.getActiveNodes(graphs[1].id);
  const campaignStoryActive = await graphService.getActiveNodes(graphs[3].id);
...
export class GraphFilteringService {
  ...
  async updateActiveStatus(
    ...
    const recentCardIds = new Set(recentRecaps.map((r) => r.id)); // Assuming Session Recap cards have IDs
...
`;
  ...
  const response = await mcpService.completion(prompt, 'You are a party-relevance filter.');
  const parsed = JSON.parse(response);
...
|------|----------|---------------|-------------------|-------|
| 1. MCP Protocol | OpenAI/Anthropic SDK direct | AsyncGenerator streaming | <10s entity extraction | No MCP wrapper needed |
...
- Feature 008 (BYOLLM Configuration): API credentials from `settings` table (BLOCKING dependency)
...
**Status**: ✓ All 10 research areas complete. Ready for Phase 1 (data model, contracts, quickstart).
