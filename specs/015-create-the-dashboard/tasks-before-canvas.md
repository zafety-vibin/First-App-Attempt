...
1. Load plan.md from feature directory
   → Extract: React 18, TypeScript 5.0+, TanStack Table v8, React Hook Form + Zod, Radix UI, React Router v6, react-grid-layout
   → Structure: Frontend + limited backend (canvas configuration persistence)
2. Load design documents:
   → data-model.md: 8 UI state models, 45+ component interfaces, 13 validation schemas, canvas state models
   → contracts/component-contracts.md: 45+ component contracts with props, renders, API calls
   → quickstart.md: 11 test scenarios (10 E2E test files including canvas)
3. Generate tasks by layer:
   → Backend Canvas Support: Migration (1), Services (2), Routes (2)
   → Foundation: Contexts (2), Utilities (3), Schemas (13 in 1 file)
   → API Services: 13 category + 2 canvas config service modules
   → Hooks: 6 original + 2 canvas hooks
   → Common Components: 5 shared UI components
   → Form Inputs: 10 field components
   → Canvas Infrastructure: 4 core components (DashboardCanvas, WidgetRegistry, WidgetPicker, BaseWidget)
   → Category Components: 16 components (landing with canvas, table, forms, detail)
   → Dashboard Widgets: 7 size-adaptive widget components
   → Sidebar: 4 navigation components
   → Pages: 6 route pages + route config
   → Tests: Component tests + E2E tests (including canvas scenarios)
4. Apply task rules:
   → Different files = mark [P] for parallel
   → Same file = sequential (no [P])
   → Backend canvas support must complete before frontend canvas
   → Implementation before tests (UI pattern, not TDD)
5. Number tasks sequentially (T001, T002...)
6. Estimated: ~116 tasks total
```
...
## Phase 3.0: Backend Canvas Support (Migration, Services, Routes)

### T001 [P]: Create dashboard canvas migration
**File**: `backend/src/db/migrations/015-dashboard-canvas.sql`
**Description**: Create SQL migration for dashboard canvas configuration persistence. Create tables: `dashboard_configs` (id, campaign_id, user_id, layout JSON, created_at, updated_at, UNIQUE(campaign_id, user_id)) and `category_landing_configs` (id, campaign_id, user_id, category, layout JSON, title, description, created_at, updated_at, UNIQUE(campaign_id, user_id, category)). Add foreign keys with CASCADE for campaign_id and user_id. Layout JSON stores react-grid-layout configuration.
**Dependencies**: None (migration file)
**Success Criteria**: Migration file created, tables defined with correct schema, foreign keys with CASCADE, unique constraints prevent duplicate configs per user+campaign
...
**Success Criteria**: Service exports 4 functions, prepared statements prevent SQL injection, layout JSON serialized/deserialized correctly, config per user+campaign enforced
...
**Dependencies**: T001 (migration creates table)
**Success Criteria**: Service exports 4 functions, prepared statements used, layout JSON + TipTap JSON stored correctly, config per user+campaign+category enforced

### T004 [P]: Create dashboardConfigs routes
**File**: `backend/src/routes/dashboardConfigs.ts`
**Description**: Create REST API routes for dashboard configs. Routes: GET /api/dashboard-configs?campaign_id={id} (get by campaign+user), POST /api/dashboard-configs (create), PUT /api/dashboard-configs/:id (update layout), DELETE /api/dashboard-configs/:id (delete). Extract user_id from auth token (Keycloak middleware from Feature 002). Validate layout JSON schema with Zod. Return 404 if config not found.
**Dependencies**: T002 (DashboardConfigService)
**Success Criteria**: 4 routes created, authentication required, layout validated with Zod, user_id extracted from token, CRUD operations work

### T005 [P]: Create categoryLandingConfigs routes
**File**: `backend/src/routes/categoryLandingConfigs.ts`
**Description**: Create REST API routes for category landing configs. Routes: GET /api/category-landing-configs?campaign_id={id}&category={category} (get by campaign+user+category), POST /api/category-landing-configs (create), PUT /api/category-landing-configs/:id (update layout/title/description), DELETE /api/category-landing-configs/:id (delete). Validate category enum, layout JSON, title max length 200, description TipTap JSON.
**Dependencies**: T003 (CategoryLandingConfigService)
**Success Criteria**: 4 routes created, authentication required, all fields validated, category enum enforced, CRUD operations work

---

## Phase 3.1: Foundation Layer (Contexts, Utilities, Schemas)

### T006 [P]: Create DashboardContext
...
### T008 [P]: Create thematicNames.ts utility
...
### T009 [P]: Create relationshipHelpers.ts utility
...
### T010 [P]: Create validationSchemas.ts
...
## Phase 3.2: API Services Layer (13 Category + 2 Canvas Config Services)

**NOTE**: Task numbers T006-T099 offset from original T001-T094 due to backend canvas tasks T001-T005 added above.

### T011 [P]: Extend apiClient.ts with view mode interceptor ✅
